		...
		<div className="container mx-auto py-8">
			...
			<div className="mb-8 flex items-center justify-between">
				<div className="flex items-center gap-3">
					<div className="rounded-lg bg-muted p-2">{server.icon}</div>
					<div>
						<h1 className="font-bold text-3xl">{server.name}</h1>
											...
											<div className="mt-4 space-y-4">
												{server.tools.map((tool) => (
													<div key={tool.id} className="rounded-lg border p-4">
														...
														<div className="inline-block rounded bg-muted px-2 py-1 text-xs">
															ソースサーバー：{tool.source}
														</div>
													</div>
												))}
