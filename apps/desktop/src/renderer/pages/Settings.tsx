...
const NotificationSection = ({
  ...
  <div className="space-y-3">
    <h3 className="text-xs font-medium text-gray-500 dark:text-zinc-500">{title}</h3>
    <div className="space-y-3">
      {items.map((item) => (
...
export const SettingsPage = (): JSX.Element => {
  ...
  return (
    <div className="space-y-4 p-6">
      <h1 className="text-lg font-semibold text-gray-900 dark:text-white">設定</h1>
      ...
      <div className="space-y-4 rounded-xl border border-gray-200 dark:border-white/[.08] bg-white dark:bg-zinc-900 p-6">
        ...
        {profile?.activeProfile === "organization" ? (
          <>
            <div className="grid gap-4 border-t border-gray-200 dark:border-white/[.08] pt-4 text-sm md:grid-cols-2">
              <div>
                <p className="text-xs text-gray-500 dark:text-zinc-500">管理サーバー</p>
                <p className="mt-1 break-all text-gray-600 dark:text-zinc-400">
                  {profile.organizationProfile?.managerUrl ?? "未設定"}
                </p>
              </div>
              <div>
                <p className="text-xs text-gray-500 dark:text-zinc-500">接続日時</p>
            ...
            <div className="flex flex-wrap items-center gap-3 border-t border-gray-200 dark:border-white/[.08] pt-4">
              <button
                type="button"
                onClick={() => void startRelogin()}
                disabled={isReloginStarting || isWaitingForRelogin}
                className="flex h-11 items-center gap-2 rounded-lg border border-gray-200 dark:border-white/[.08] px-4 text-sm font-medium text-gray-600 dark:text-zinc-400 transition hover:bg-black/[.02] dark:bg-white/[.04] disabled:cursor-not-allowed disabled:opacity-50"
              >
            ...
            <div className="space-y-4 border-t border-gray-200 dark:border-white/[.08] pt-4">
              <div>
              ...
              {!sessionError && !desktopSession && (
                <div className="rounded-lg border border-gray-200 dark:border-white/[.08] bg-black/[.06] dark:bg-white/[.08] p-4 text-sm text-gray-500 dark:text-zinc-500">
                  {sessionLoading
                    ? "管理サーバーから取得しています..."
                    : "組織セッションはまだ取得されていません。"}
                </div>
              ...
              {desktopSession && (
                <>
                  <div className="grid gap-3 md:grid-cols-4">
                    <div className="rounded-lg border border-gray-200 dark:border-white/[.08] bg-black/[.06] dark:bg-white/[.08] p-3">
                  ...
                  <div className="grid gap-4 lg:grid-cols-2">
                    <div className="rounded-lg border border-gray-200 dark:border-white/[.08] p-4">
                    ...
                    {desktopSession.groups.length > 0 ? (
                      <div className="mt-3 grid gap-2 md:grid-cols-2">
                        {desktopSession.groups.map((group) => (
                          <div
                            key={group.id}
                            className="rounded-lg bg-black/[.06] dark:bg-white/[.08] px-3 py-2"
                          >
                            <div className="flex items-center justify-between gap-2">
                              <p className="truncate text-sm text-gray-600 dark:text-zinc-400">
                                {group.name}
                              </p>
                              <span className="rounded-full bg-black/[.02] dark:bg-white/[.04] px-2 py-0.5 text-[10px] text-gray-400 dark:text-zinc-600">
                                {group.source}
                              </span>
                            </div>
                            <p className="mt-1 truncate text-[10px] text-gray-400 dark:text-zinc-600">
                      ...
                      <p className="mt-3 rounded-lg bg-black/[.06] dark:bg-white/[.08] px-3 py-2 text-sm text-gray-500 dark:text-zinc-500">
                        所属グループなし
                      </p>
                    )}
                  </div>
                          ...
                          >
                            <div className="flex items-center justify-between gap-2">
                              ...
                              <div className="flex shrink-0 items-center gap-1">
                                {orgUnit.isPrimary && (
                                  <span className="rounded-full bg-blue-600/10 dark:bg-blue-400/10 px-2 py-0.5 text-[10px] text-blue-600 dark:text-blue-400">
                                    primary
                                  </span>
                                )}
                                <span className="rounded-full bg-black/[.02] dark:bg-white/[.04] px-2 py-0.5 text-[10px] text-gray-400 dark:text-zinc-600">
                  ...
                  <div className="rounded-lg border border-gray-200 dark:border-white/[.08] p-4">
                    ...
                    <div className="mt-3 flex flex-wrap gap-3 text-sm">
                      <div className="rounded-lg border border-gray-200 dark:border-white/[.08] px-4 py-2.5">
                      ...
                      {permissionEntries.map(([perm, count]) => (
                        <div
                          key={perm}
                          className="rounded-lg border border-gray-200 dark:border-white/[.08] px-4 py-2.5"
                        >
                          <p className="text-xs text-gray-500 dark:text-zinc-500">
                            {perm}
                          </p>
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-2">
                      {Object.entries(desktopSession.features).map(
                        ([key, enabled]) => (
                          <div
                            key={key}
                            className="flex items-center justify-between rounded-lg bg-black/[.06] dark:bg-white/[.08] px-3 py-2"
                          >
                            ...
                            <span
                              className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${
                                enabled
                                  ? "bg-emerald-500/10 dark:bg-emerald-400/10 text-emerald-600 dark:text-emerald-400"
                                  : "bg-black/[.02] dark:bg-white/[.04] text-gray-400 dark:text-zinc-600"
                              }`}
                            >
                              {enabled ? "ON" : "OFF"}
                            </span>
          ...
          <div className="rounded-lg border border-gray-200 dark:border-white/[.08] bg-black/[.06] dark:bg-white/[.08] p-4">
            ...
            <button
              ...
              className="mt-4 inline-flex items-center gap-2 rounded-lg border border-gray-200 dark:border-white/[.08] px-4 py-2 text-sm font-medium text-gray-600 dark:text-zinc-400 transition hover:bg-black/[.02] dark:bg-white/[.04] hover:text-gray-900 dark:text-white"
            >
              <Building2 size={16} />
              組織利用へ切り替え
            </button>
      ...
      {/* 通知設定 */}
      <div className="space-y-5 rounded-xl border border-gray-200 dark:border-white/[.08] bg-white dark:bg-zinc-900 p-6">
        <h2 className="text-sm font-medium text-gray-900 dark:text-white">
          通知設定
        </h2>
        <NotificationSection
      ...
      <button
        type="button"
        className="rounded-lg bg-gray-900 dark:bg-white px-4 py-2 text-sm font-medium text-white dark:text-black transition-colors hover:opacity-90"
      >
        保存
      </button>
