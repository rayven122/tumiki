...
const handleMcpOAuthCallback = async (url: string): Promise<void> => {
  ...
  if (!mcpOAuthManager) {
    ...
    sendToWindow("oauth:error", "OAuth認証マネージャーが初期化されていません");
    return;
  }
...
app
  ...
  .then(async () => {
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      logger.info(
        "Dev: mcp-bridge.json は MCP 起動成功後にここへ書かれます（パッケージ名によりフォルダ名が @tumiki/desktop とは限りません）",
        {
          userData,
          mcpBridgeJson: pathJoin(userData, "mcp-bridge.json"),
        },
      );
    }
    ...
    // Claude 等: stdio ブリッジが接続する先（mcp-process 内 TCP）を書き出す
    setBridgeReadyListener(({ host, port }) => {
      try {
        const userData = app.getPath("userData");
        mkdirSync(userData, { recursive: true });
        const file = pathJoin(userData, "mcp-bridge.json");
        writeFileSync(
          file,
          JSON.stringify(
            {
        ...
        logger.info(
          "Wrote mcp-bridge.json for stdio bridge (tumiki-mcp-bridge)",
          {
            file,
            host,
            port,
        ...
        logger.error("Failed to write mcp-bridge.json", {
          error: error instanceof Error ? error.message : error,
        });
      }
    });
