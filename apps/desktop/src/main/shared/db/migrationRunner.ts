 ...
 *
 * 注意: 単純なセミコロン分割・インラインコメント除去のため、SQLリテラル内に
 * セミコロンや「--」が含まれる場合は誤分割・誤除去される。
 * （例: DEFAULT 'foo--bar' → DEFAULT 'foo に切り詰め）
 * Prismaが生成するDDL SQLに限っては問題ないが、
 * 文字列リテラルにこれらを含むデフォルト値を追加した際は要注意。
 */
