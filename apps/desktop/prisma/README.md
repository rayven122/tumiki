...
**Properties**
  ...
  - `disableRuntimeRefresh`
    > ランタイム自動リフレッシュを無効化するか。
    > 一部プロバイダ (Figma, Atlassian 等) で refresh_token grant が MCP の audience/scope を
    > 保持しないトークンを返すケースがあり、ランタイムリフレッシュが上流401を招くため。
    > true の場合は起動時の refreshOAuthTokenIfNeeded のみが走り、ランタイム resolveHeaders は静的ヘッダーのまま。
  - `createdAt`: 
  - `updatedAt`: 