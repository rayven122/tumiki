...
- [McpServer](#mcpserver)
- [OAuth](#oauth)
- [AiCodingTelemetry](#aicodingtelemetry)
...
### `AiCodingMetric`
AI コーディングツールのメトリクス（Claude Code / Codex CLI）

**Properties**
  - `id`: 
  - `tool`: ツール識別子（"claude-code" | "codex"）
  - `metricName`: メトリクス名（例: "claude_code_tokens_total"）
  - `value`: 
  - `attributes`: 追加属性（JSON 文字列、例: {"type":"input"}）
  - `recordedAt`: 

### `AiCodingTrace`
AI コーディングツールのトレース（Claude Code / Codex CLI）

**Properties**
  - `id`: 
  - `tool`: ツール識別子（"claude-code" | "codex"）
  - `traceId`: OTLP トレース ID
  - `spanName`: スパン名（例: "llm.call"）
  - `durationMs`: スパンの所要時間（ミリ秒）
  - `attributes`: 追加属性（JSON 文字列）
  - `startedAt`: 