...
- ツールカタログから「利用許可済みツール」をワンクリックで全社配布
...
| 目標 | 利用許可済みのMCPツールを手軽に使い始めたい                  |
| 課題 | どのツールが使えるか分からない、設定が難しい                 |
...
- `利用可` → 「追加する」で自分の環境に即時追加
- `利用中` → 追加済み。クリックで接続情報を表示
- `利用不可` → 申請導線は出さず、管理者側の権限設定で制御（申請フローは提供対象外）
...
社員がアプリストア感覚で利用許可済みツールを閲覧・追加するUI。
