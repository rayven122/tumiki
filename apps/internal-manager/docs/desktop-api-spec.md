...
`policyVersion` は opaque string として扱う。Manager 側はポリシー更新時刻と内部バージョンカウンタから生成し、Desktop は値を比較せず、次回 `GET /api/desktop/v1/policy-sync?since=...` にそのまま渡す。例: `2026-05-03T10:00:00.000Z-42`。
...
現行 internal-manager の Prisma ID に合わせ、Manager が返す `requestId` は CUID/CUID2 形式を想定する。Desktop が生成する `clientRequestId` とは別物として扱う。
...
```http
GET /api/desktop/v1/access-requests?limit=50&cursor=...
```
