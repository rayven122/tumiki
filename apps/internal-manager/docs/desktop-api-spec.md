...
将来的には Desktop 向け API の prefix を `/api/desktop` に統一するため、`POST /api/desktop/audit-logs` を追加し、既存の `/api/internal/audit-logs` は互換 endpoint として残す。

## 今後必要になる API

特記がない限り、以下の API はすべて Bearer token 認証を必須とする。

カタログ ID は DB の連番ではなく、Desktop と internal-manager の間で安定して共有できる文字列 ID を使う。例: `github`, `slack`, `google-drive`。
...
ページネーションは cursor-based とし、`limit` のデフォルトは 100、最大は 500 とする。
...
| catalog ID の管理  | 安定した文字列 catalog ID の登録元と重複防止ルール                             |
