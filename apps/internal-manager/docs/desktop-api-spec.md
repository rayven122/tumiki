...
将来的には Desktop 向け API の prefix を `/api/desktop` に統一するため、`POST /api/desktop/audit-logs` を追加し、既存の `/api/internal/audit-logs` は `v2.0.0` リリースまで互換 endpoint として残す。
...
`policyVersion` は opaque string として扱う。Manager 側はポリシー更新時刻と内部バージョンカウンタから生成し、Desktop は値を比較せず、次回 `GET /api/desktop/policy-sync?since=...` にそのまま渡す。
