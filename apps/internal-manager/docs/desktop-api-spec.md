...
現行実装で使っている API は、OIDC 設定取得と監査ログ送信のみ。今後は Desktop 側に「組織で許可された MCP を配布する」「権限・カタログ変更を同期する」ための API が必要になる。
...
組織・グループ・ユーザー個別のポリシーで利用可能な MCP カタログ一覧を返す API。権限がない、または停止中のカタログは `disabled` として返し、申請導線は出さない。
...
承認・アクセス申請フローは今回の提供対象外のため、`/api/desktop/v1/access-requests` は提供しない。

ユーザー個別の例外権限は管理者が internal-manager で設定する。申請作成・申請状態同期 API を実装する場合は、別 PR で DB migration、API、管理画面、Desktop 導線をまとめて設計する。
...
Desktop はオフライン中も監査ログをローカルに蓄積し、オンライン復帰後に再送する。
