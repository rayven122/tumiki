...
現行実装で使っている API は、OIDC 設定取得と監査ログ送信のみ。今後は Desktop 側に「組織で許可された MCP を配布する」ための API が必要になる。
...
組織で利用可能な MCP カタログ一覧を返す API。権限がない、または停止中のカタログは `disabled` として返し、申請導線は出さない。
...
`IndividualPermission` / `ApprovalStatus` は承認済みの個別許可を Desktop セッションへ反映する既存モデルとして残す。申請作成・申請状態同期 API を実装する場合は、別 PR で DB migration と管理画面を含めて設計する。
