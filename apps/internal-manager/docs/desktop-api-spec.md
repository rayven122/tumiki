...
- 本番運用までに `audience = OIDC_CLIENT_ID` の検証を追加する。未対応期間は internal-manager への到達経路を信頼済みネットワークに限定してリスクを下げる。
...
将来的には Desktop 向け API の prefix を `/api/desktop` に統一するため、`POST /api/desktop/audit-logs` を追加し、既存の `/api/internal/audit-logs` は次のメジャーリリースまで互換 endpoint として残す。
...
`policyVersion` は opaque string として扱う。Desktop は値を比較せず、次回 `GET /api/desktop/policy-sync?since=...` にそのまま渡す。
...
```http
GET /api/desktop/catalogs?limit=50&cursor=...
```
...
```http
GET /api/desktop/access-requests?limit=50&cursor=...
```
...
推奨ポーリング間隔は 5 分以上とする。サーバー側はユーザー単位で rate limit を設定し、短時間の連続同期を抑制する。
...
```http
GET /api/desktop/audit-logs?from=...&to=...&mcpServerId=...&limit=100&cursor=...
```
...
| 項目               | 判断が必要な内容                                                                                                                                                  |
| ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| JWT audience 検証  | **実装前に必須対応**。`verifyDesktopJwt()` に `aud = OIDC_CLIENT_ID` 検証を追加する。対応完了までは `/api/desktop/*` への到達経路を信頼済みネットワークに限定する |
| 認証情報配布       | Manager から secret を配布しない方針を維持するか、管理配布する場合の暗号化方式                                                                                    |
| 再送上限           | 監査ログ・申請の自動再送を何回まで行うか                                                                                                                          |
| API バージョニング | `/api/desktop/v1/...` を採用するか、既存 prefix のまま後方互換で運用するか                                                                                        |
