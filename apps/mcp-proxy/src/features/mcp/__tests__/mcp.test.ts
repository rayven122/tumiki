...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    mcpRequestHandler: vi.fn((c: { json: (body: unknown) => Response }) =>
      c.json({ result: "ok" }),
    ),
  };
});
...
vi.mock("../mcpRequestHandler.js", () => ({
  mcpRequestHandler: mocks.mcpRequestHandler,
}));

import { mcpFeatureRoute } from "../route.js";

describe("mcpRoute", () => {
  test("mcpRouteが定義されている", () => {
    expect(mcpFeatureRoute).toBeDefined();
  });

  test("POST /mcp/:mcpServerIdルートがステータス200を返す", async () => {
    const res = await mcpFeatureRoute.request("/mcp/test-server-id", {
      method: "POST",
  ...
  test("mcpServerIdなしのパスは404を返す", async () => {
    const res = await mcpFeatureRoute.request("/mcp/", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ test: true }),
    });
  ...
  test("各ミドルウェアがPOSTリクエストで呼ばれる", async () => {
    ...
    await mcpFeatureRoute.request("/mcp/test-server-id", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ test: true }),
    });
  ...
  test("mcpRequestHandlerがPOSTリクエストで呼ばれる", async () => {
    mocks.mcpRequestHandler.mockClear();
    ...
    expect(mocks.mcpRequestHandler).toHaveBeenCalled();
  });
});
