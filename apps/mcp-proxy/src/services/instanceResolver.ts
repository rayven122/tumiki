 ...
 * @returns McpServer
 * @throws Error - mcp_instance_id がない、サーバーが見つからない、削除済み、組織不一致の場合
 */
export const resolveMcpServer = async (
  ...
  if (!mcpServer) {
    ...
    throw new Error(
      `MCP server not found: ${mcp_instance_id}. Please verify your JWT token.`,
    );
  }
  ...
  if (mcpServer.deletedAt !== null) {
    ...
    throw new Error(
      `MCP server is deleted: ${mcp_instance_id}. Please contact support.`,
    );
  }
  ...
  if (mcpServer.organizationId !== org_id) {
    ...
    throw new Error(
      `Organization ID mismatch for server ${mcp_instance_id}. Access denied.`,
    );
  }
