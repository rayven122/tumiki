...
// SpeechQueue のシングルトンインスタンス
let speechQueueInstance: SpeechQueue | null = null;
...
export const useCoharuContext = () => {
  ...
  const getSpeechQueue = useCallback(() => {
    if (!speechQueueInstance) {
      speechQueueInstance = new SpeechQueue({
        onPlayStart: () => setIsSpeaking(true),
        onPlayEnd: () => setIsSpeaking(false),
      });
    }
    return speechQueueInstance;
  }, [setIsSpeaking]);
