...
export const Header = ({
  ...
  const loginSlug = session?.user?.defaultOrganization?.slug;
  const isLoggedIn = !!loginSlug;
  const ctaHref = isLoggedIn ? `/${loginSlug}/mcps` : "/signup";
  const ctaLabel = isLoggedIn ? "ダッシュボード" : "はじめる";
  return (
