...
type ExpirationDisplayProps = {
  oauthTokenStatus?: OAuthTokenStatus | null;
  apiKeyStatus?: ApiKeyStatus | null;
};

export const ExpirationDisplay = ({
  oauthTokenStatus,
  apiKeyStatus,
}: ExpirationDisplayProps) => {
  // 表示するものがない場合は何も表示しない
  if (!oauthTokenStatus?.hasToken && !apiKeyStatus) {
    return null;
  }
  ...
  return (
    <div className="flex flex-col gap-1.5 text-xs">
      {/* OAuth トークンの有効期限 */}
      {oauthTokenStatus?.hasToken && (
        <div className="flex items-center gap-1.5">
          <Clock className="h-3.5 w-3.5 text-gray-500" />
          <span className="font-medium text-gray-700">MCP OAuth</span>
          <span className="text-gray-400">|</span>
          <span
            className={cn(
              "font-semibold",
              oauthTokenStatus.isExpired ||
                (oauthTokenStatus.daysRemaining !== null &&
                  oauthTokenStatus.daysRemaining <= 1)
                ? "text-red-600"
                : "text-gray-700",
            )}
          >
