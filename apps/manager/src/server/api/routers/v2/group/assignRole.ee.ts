...
import type { AssignRoleToGroupInput } from "../../../../utils/groupSchemas";
import {
...
export const assignRoleToGroup = async (
  ...
  if (currentOrg.id !== input.organizationId) {
    throw new TRPCError({
      code: "FORBIDDEN",
      message: "他の組織のグループにロールを割り当てることはできません",
    });
  }
  ...
  // グループにロールマッピングを追加
  const result = await keycloakProvider.addRoleMappingToGroup({
  ...
  if (!result.success) {
    throw new TRPCError({
      code: "INTERNAL_SERVER_ERROR",
      message: result.error ?? "ロールの割り当てに失敗しました",
    });
  }
