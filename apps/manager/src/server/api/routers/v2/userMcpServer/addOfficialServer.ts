...
import type { AddOfficialServerInputV2 } from ".";
import { AuthType, ServerStatus, ServerType } from "@tumiki/db/server";
import type { PrismaTransactionClient } from "@tumiki/db";
import { TRPCError } from "@trpc/server";

export type AddOfficialServerInput = z.infer<typeof AddOfficialServerInputV2>;

export type AddOfficialServerOutput = {
  id: string;
  mcpConfigId: string;
};

/**
 * MCPサーバーを追加（テンプレートベースまたはカスタムURL）
 *
...
export const addOfficialServer = async (
  ...
  userId: string,
): Promise<AddOfficialServerOutput> => {
