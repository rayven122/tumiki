  ...
  return (
    <DropdownMenu open={open} onOpenChange={setOpen}>
      ...
      >
        ...
        >
          {isAutoSelected && <SparklesIcon size={12} />}
          {selectedDisplayName}
          <ChevronDownIcon />
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="min-w-[280px]">
        {/* 自動選択オプション */}
        <DropdownMenuItem
          data-testid="compact-model-selector-auto"
        ...
        {groupedModels.map((group) => (
          <DropdownMenuSub key={group.provider}>
            <DropdownMenuSubTrigger
              data-testid={`compact-model-selector-provider-${group.provider}`}
            >
              {group.label}
            </DropdownMenuSubTrigger>
            <DropdownMenuSubContent className="min-w-[280px]">
              {group.models.map((chatModel) => {
                ...
                return (
                  <DropdownMenuItem
                    key={chatModel.id}
                    data-testid={`compact-model-selector-item-${chatModel.id}`}
