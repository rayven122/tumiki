# DEV-1477: MCPカタログ画面 OAuth認証設定機能
...
```
oauth/
├── oauth.service.ts       # メインのOAuthフローオーケストレーション
├── oauth.ipc.ts           # IPCハンドラー
├── oauth.repository.ts    # OAuthClient CRUD
├── oauth.types.ts         # 型定義
├── discovery.ts           # RFC 9728 + RFC 8414 メタデータ取得
├── dcr.ts                 # RFC 7591 Dynamic Client Registration
├── pkce.ts                # PKCE (code_verifier, code_challenge)
├── protocol-handler.ts    # カスタムプロトコル（tumiki://）ハンドラー
└── token-exchange.ts      # 認可コード → トークン交換
```
...
|-----------|-----------|------|
| `lib/oauth/dcr.ts` | `oauth/discovery.ts` + `oauth/dcr.ts` | ほぼそのまま参考にできる |
| `lib/oauth/pkce.ts` | `oauth/pkce.ts` | そのまま流用可能 |
| `lib/oauth/oauth-client.ts` | `oauth/token-exchange.ts` | トークン交換ロジックを流用 |
...
→ **対応**: `client_name` を `"Claude Code"` に設定（Manager実装と同一）。

### 2. DCR非対応サーバー（GitHub）

GitHub MCP (`https://api.githubcopilot.com/mcp`) は Discovery (RFC 9728 + 8414) には対応しているが、
...
→ **対応方針**: カタログのシードデータに `oauthClientId` / `oauthClientSecret` フィールドを追加。
  事前に GitHub Developer Settings で OAuth App を作成し、client_id/secret をシードデータに埋め込む。
  DCR が利用できない場合、シードデータの client_id/secret をフォールバックとして使用する。

### 3. サーバー対応状況一覧

| サーバー | Discovery | DCR | カスタムプロトコル | 状態 |
|---------|-----------|-----|-------------------|------|
| Linear | RFC 8414 | 対応 | 対応 | **全自動で動作確認済み** |
| Figma | RFC 9728 + 8414 | 対応（client_name制限あり） | 対応 | **client_name="Claude Code"で動作確認済み** |
| GitHub | RFC 9728 + 8414 | 非対応 | 対応 | **client_id手動設定で動作確認済み** |
| その他 | 未検証 | 未検証 | 未検証 | |

### 4. DCR非対応サーバーへの対応設計

```
oauth.service.startOAuthFlow(input)
  +-- 1. OAuthClient テーブルにキャッシュがあるか検索
  |   +-- キャッシュあり → client_id/secret を使用（DCR/手動問わず）
  |   +-- キャッシュなし → 2へ
  +-- 2. Discovery でメタデータ取得
  +-- 3. registration_endpoint の有無を確認
  |   +-- あり → DCR 実行 → 結果をキャッシュ保存
  |   +-- なし → 4へ（DCR非対応フォールバック）
  +-- 4. カタログの oauthClientId/secret を確認
  |   +-- あり → そのまま使用（キャッシュ保存）
  |   +-- なし → エラー「このサーバーはOAuth自動登録に対応していません」
  +-- 5. 以降のフローへ（PKCE → ブラウザ → トークン交換）
```

実装変更箇所:
- `apps/desktop/prisma/schema.prisma` — `McpCatalog` に `oauthClientId` / `oauthClientSecret` カラム追加
- `apps/desktop/src/main/features/catalog/catalog.repository.ts` — `CatalogSeedData` 型に同フィールド追加
- `apps/desktop/src/main/features/catalog/seed-data.ts` — GitHub MCP に環境変数から client_id/secret 設定
- `apps/desktop/src/main/features/oauth/oauth.types.ts` — `StartOAuthInput` / `McpOAuthSession` にフォールバック情報追加
- `apps/desktop/src/main/features/oauth/oauth.service.ts` — `getOrRegisterClient` で DCR 非対応時のフォールバック
- `apps/desktop/src/types/catalog.ts` — `CatalogItem` 型に同フィールド追加
- `apps/desktop/electron.vite.config.ts` — `define` に `GITHUB_MCP_CLIENT_ID` / `GITHUB_MCP_CLIENT_SECRET` 追加
- `.env` — `GITHUB_MCP_CLIENT_ID` / `GITHUB_MCP_CLIENT_SECRET` を設定
...
```typescript
{
  ...
  ...
  authType: "OAUTH",
  // DCR非対応サーバー用: 事前登録済みOAuthクライアント
  oauthClientId: process.env.GITHUB_MCP_CLIENT_ID,
  oauthClientSecret: process.env.GITHUB_MCP_CLIENT_SECRET,
}
```

### 5. oauth4webapi バンドル設定
...
1. `pnpm --filter desktop db:migrate dev` でマイグレーション適用
2. デスクトップアプリ起動（`pnpm --filter desktop dev`）
3. カタログ画面でOAuthアイテム（例: Figma MCP）の「追加」をクリック
4. 「ブラウザで認証」ボタンクリック → ブラウザが開く
5. OAuth認証完了 → デスクトップアプリに戻る
6. MCPサーバー一覧（/tools）に新しいサーバーが表示される
