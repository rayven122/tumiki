...
|------|-------------------|---------------|
| **ロギング** | Google Cloud Pub/Sub → BigQuery | PostgreSQL直接保存 |
| **PIIマスキング** | GCP DLP | Regexベース実装 |
| **認証** | Cloud Run IAM | 削除（Keycloak JWT利用） |
| **依存パッケージ** | `@google-cloud/dlp`, `@google-cloud/pubsub` | 削除 |
