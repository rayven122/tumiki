...
#### Google Cloud依存の削除・オプション化

| 項目 | 従来（Cloud Run版） | Self-Hosted版 |
|------|-------------------|---------------|
| **ロギング** | Google Cloud Pub/Sub → BigQuery | ローカルファイル（メイン）+ PostgreSQL（統計のみ） |
| **PIIマスキング** | GCP DLP | Regexベース（デフォルト）+ GCP DLP（オプション） |
| **認証** | Cloud Run IAM | オプション化（環境変数で無効化） |
| **依存パッケージ** | `@google-cloud/dlp`, `@google-cloud/pubsub` | Pub/Sub削除、DLP/Authはオプション |
