...
- **カバレッジ**: vitest を使って実装ロジックのカバレッジ100%を目標
...
このプロジェクトはモノレポ構成のため、個別のアプリケーションやパッケージを単独で起動した場合、依存しているパッケージ（`@tumiki/*` パッケージ）の変更がリアルタイムに反映されません。

### 制限事項
- 個別アプリ（例：`apps/manager`）を `pnpm dev` で起動した場合、`packages/*` 内のファイル変更は自動反映されない
- パッケージの変更を反映するには、該当パッケージのビルドが必要

### 推奨される開発方法
1. **プロジェクトルートから全体を起動する**：`pnpm dev` をプロジェクトルートで実行
2. **個別起動する場合**：依存パッケージで `pnpm dev` を実行してwatchモードにする
   ```bash
   # packages/db で変更を監視
   cd packages/db && pnpm dev
   # 別ターミナルで apps/manager を起動
