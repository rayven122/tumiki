...
- `auth.prisma` - Better Auth認証テーブル (Keycloak統合)
...
- `oauth.prisma` - OAuth認証関連（Backend MCP用）
...
- **MCP プロキシサーバー**: `apps/proxyServer/src/index.ts`
  ...
  - `/oauth` - OAuth 2.0 ディスカバリーエンドポイント
- **型安全性**: 自動 API 生成によるフルスタック型安全性、@tumiki/db から型 import

### 認証アーキテクチャ

#### 第1層: Tumiki認証 (Keycloak)
- **プロバイダー**: Keycloak OIDC統合
- **パッケージ**: `@tumiki/better-auth` (Better Auth v1.3.4)
- **設定**: `packages/better-auth/src/config.ts`
- **セッション**: JWT ベース、Keycloakセッション管理
- **API Routes**: `apps/manager/src/app/api/auth/[...all]/route.ts`
- **ユーザーマッピング**: `keycloakId`, `role`, `email`, `name`, `image`

#### 第2層: Backend MCP OAuth
- **トークン管理**: `@tumiki/oauth-token-manager` パッケージ
- **キャッシュ**: Redisを使用した高速トークン取得
- **自動リフレッシュ**: 有効期限5分前の自動リフレッシュ
- **暗号化**: アクセストークン/リフレッシュトークンの暗号化保存

### Keycloak統合状況

**実装完了項目**:
- ✅ Better Auth設定 (Keycloak OAuth Provider)
- ✅ データベーススキーマ (User.keycloakId, Better Authテーブル)
- ✅ OAuth Token Manager (トークン管理、キャッシュ、リフレッシュ)
- ✅ Manager App認証統合

**残りの実装** (詳細は `docs/keycloak-implementation-status.md` 参照):
- 🔨 Keycloak環境構築 (Docker Compose)
- 🔨 Proxy Server JWT検証ミドルウェア
- 🔨 OAuth Endpoints (`/oauth/register`, `/oauth/token`, `/oauth/revoke`)
- 🔨 Manager UI OAuth設定画面
- 🔨 監査ログシステム
- 🔨 テスト実装 (単体・統合)
...
現在、Auth0からKeycloakへの移行を進行中です。詳細な実装状況と残タスクについては以下のドキュメントを参照してください：

- **実装状況**: [docs/keycloak-implementation-status.md](./docs/keycloak-implementation-status.md)
- **実装計画**: [docs/keycloak-onpremise-implementation.md](./docs/keycloak-onpremise-implementation.md)
- **二層OAuth設計**: [docs/auth/two-tier-oauth-implementation-plan.md](./docs/auth/two-tier-oauth-implementation-plan.md)

### 実装完了項目
- ✅ `@tumiki/better-auth` パッケージ (Keycloak OIDC統合)
- ✅ `@tumiki/oauth-token-manager` パッケージ (トークン管理)
- ✅ データベーススキーマ (Better Auth + OAuth)
- ✅ Manager App認証統合

### 優先実装タスク
1. 🔴 Keycloak環境構築 (Docker Compose)
2. 🔴 Proxy Server JWT検証ミドルウェア
3. 🔴 OAuth Endpoints実装
4. 🟡 Manager UI OAuth設定画面
5. 🟡 監査ログシステム
6. 🟡 テスト実装
