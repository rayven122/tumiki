...
├── users.tf             # テストユーザー
├── authentication-flows.tf # Google-only browser flow（オプション）
...
### 本番 Google-only 認証と break-glass

本番環境は `terraform.tfvars.production` で `google_only_browser_login = true` とし、通常のブラウザログインを Google IdP のみに限定します。ローカルのメールアドレス + パスワードフォームは、Google IdP が設定されている限りテーマ上も表示しません。

Google IdP 障害時は master realm の管理コンソール、または `admin@tumiki.cloud` の direct grant 用 break-glass ユーザーで復旧します。`google_only_browser_login = false` で apply すると browser flow は Keycloak デフォルトへ戻りますが、Google IdP 自体が realm に残る限り通常ログイン画面は Google ボタンのみを表示します。ローカルフォームを通常ユーザーへ開放する運用は想定していません。
...
- `terraform.tfvars.staging` - ステージング環境用
...
ステージングの Keycloak 設定は次のコマンドで適用します。

```bash
pnpm keycloak:staging:plan
...
| コマンド                      | 説明                       |
| ----------------------------- | -------------------------- |
| `pnpm keycloak:staging:plan`  | ステージング変更プレビュー |
| `pnpm keycloak:staging:apply` | ステージング設定適用       |
