...
"McpTool" {
  ...
  String mcpConnectionId FK "nullable"
  Boolean isAllowed
  McpToolReviewStatus reviewStatus
  DateTime discoveredAt "nullable"
  DateTime lastSeenAt "nullable"
  String schemaHash "nullable"
  DateTime createdAt
  DateTime updatedAt
}
...
"McpTool" }o--o| "McpServerTemplate" : mcpServerTemplate
"_McpServerTemplateInstanceToMcpTool" }o--|| "McpTool" : McpTool
```

### `McpCatalog`
MCPカタログ（接続テンプレート）
Desktop側のMcpCatalogと同じ概念で、管理画面では「MCPカタログ」として扱う

**Properties**
  - `id`:
  - `name`: カタログ表示名
  - `slug`: URL識別子（組織内でユニーク）
  - `description`: カタログ説明
  - `iconPath`: アイコンパス
  - `transportType`: 接続タイプ
  - `command`: STDIO用コマンド
  ...
  - `credentialKeys`: 必要な設定キー名（STDIO: 環境変数キー / SSE・Streamable HTTP: ヘッダー名）
  - `authType`: 認証タイプ
  - `organizationId`: 組織ID（公式カタログはOFFICIAL_ORGANIZATION_ID、組織専用カタログは組織ID）
  - `createdById`: 作成者
  - `createdAt`:
  - `updatedAt`:
...
**Properties**
  ...
  - `mcpServerTemplateId`: 旧テンプレートベースのツール定義（互換用）
  - `mcpConnectionId`: Desktop寄せの接続ベースのツール定義
  - `isAllowed`: 組織全体として公開対象にするか。falseなら常に拒否。
  - `reviewStatus`: 管理者レビュー状態。新規/変更ツールはレビュー待ちにする。
  - `discoveredAt`: 初回発見日時
  - `lastSeenAt`: 最終確認日時
  - `schemaHash`: inputSchema等の変更検知用ハッシュ
  - `createdAt`:
  - `updatedAt`:
...
"McpConnection" }o--|| "McpServer" : mcpServer
...
Desktop側のMcpConnectionと同じ概念で、credentialsに接続用credential/configを暗号化保存する

**Properties**
  - `id`:
  - `name`: 接続表示名
  - `slug`: URL識別子（McpServer内でユニーク）
  - `mcpServerId`: 所属するMCPサーバー
  ...
  - `url`: SSE/Streamable HTTP用URL
  - `credentialKeys`: 必要な設定キー名（STDIO: 環境変数キー / SSE・Streamable HTTP: ヘッダー名）
  - `credentials`: 接続設定値（STDIO: 環境変数 / SSE・Streamable HTTP: HTTPヘッダー / OAuth token）
  - `authType`: 認証タイプ
  - `isEnabled`: 有効/無効フラグ
...
Desktop側のOAuthClientと同じ概念で、MCP OAuth認証時のDynamic Client Registration結果を保存する

**Properties**
  - `id`:
  - `serverUrl`: MCPサーバーURL（正規化済み、認可サーバー特定用）
  - `issuer`: OAuth Authorization Server issuer URL
  - `clientId`: DCRで取得したクライアントID
  - `clientSecret`: DCRで取得したクライアントシークレット
