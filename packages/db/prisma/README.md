...
- `id`:
- `mcpServerTemplateId`: スキャン対象のMCPサーバーテンプレートID
...
"McpServerTemplate" {
  ...
  String organizationId FK
  Int currentToolVersion "nullable"
  DateTime lastScannedAt "nullable"
  DateTime createdAt
  DateTime updatedAt
}
"McpTool" {
...
- `currentToolVersion`
  > 現在 McpTool に反映されているバージョン（null = 未スキャン、管理者承認済み）
  > CLEAN なバージョンのみがここに設定される（Git の HEAD 相当）
- `lastScannedAt`: 最終スキャン日時（変更有無に関わらずスキャン実行時に更新）
- `createdAt`:
- `updatedAt`:
